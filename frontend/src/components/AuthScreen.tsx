...
// ============================================================================
// Handles both user login and registration. Uses React Query for API mutations
// and Zustand for global state management upon successful authentication.
...
export const AuthScreen = () => {
  // Toggle state between Login and Registration mode
  const [isLogin, setIsLogin] = useState(true);
  
  // Extract the login action from global Zustand store
  ...
  // React Query Mutation: Handles both API calls dynamically based on 'isLogin' state
  const authMutation = useMutation({
    mutationFn: (data: typeof formData) => isLogin ? loginUser(data) : registerUser(data),
    onSuccess: (data) => {
      // Save JWT token and user data to global store (Zustand + LocalStorage)
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <input 
              type="password" 
              required
              minLength={8} // Built-in HTML validation matches backend requirements of at least 8 characters
              value={formData.password}
              onChange={(e) => setFormData({...formData, password: e.target.value})}
              className="w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg p-3 outline-none focus:ring-2 focus:ring-blue-500 transition-shadow" 
              placeholder="********" 
            />
            {/* Password Policy Helper - Only visible during registration */}
            {!isLogin && (
